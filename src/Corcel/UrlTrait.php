...
namespace LaravelAdmin\HeadlessWordpress\Corcel;
	...
	public static function url($url, $abort=true)
	{
		...
		$selected = $pages->filter(function($page) use ($segments)
		{
			...
			foreach ($segments as $segment)
			{
				...
				//	if not return false
				if (!$current || $current->slug != $segment) return false;
			}
		...
		if ($return = $selected->first()) return $return;
